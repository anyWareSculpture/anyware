# anyWare Coding Style

This document is intented to provide consistent and helpful coding convention for our ES6 code. It should be usable either in Node/IO.js or in the browser.
See [CODING STANDARDS](CODING_STANDARDS.md) for what we expect from the code we write.
...
- Static strings **must** be declared using single quotes, double quotes or backticks.
...
- 1 space **must** be used before the ending brace `}` in single-line object literals or function declaration. **Avoid** using single line function literals when you can.
